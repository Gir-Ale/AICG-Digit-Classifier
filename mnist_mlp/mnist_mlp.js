...
const r_16_4_4_8_64 = `fn nan() -> f32 { let bits = 0xffffffffu; return bitcast<f32>(bits); }
@group(0) @binding(0)
var<uniform> INFINITY : f32;
var<workgroup> temp0: array<f32,128>;
@group(0) @binding(1)var<storage,read_write>data0_256:array<f32>;
...
@compute @workgroup_size(4,8) fn main(@builtin(workgroup_id) gindex: vec3<u32>,@builtin(local_invocation_id) lindex: vec3<u32>) {
  ...
  var alu4 = (lidx0+bitcast<i32>(precast3));
  var val0 = data3_256[alu4];
  var alu5 = (alu4+4);
  var val1 = data3_256[alu5];
  var alu6 = (alu4+8);
  var val2 = data3_256[alu6];
  var alu7 = (alu4+12);
  var val3 = data3_256[alu7];
  acc0[0] = 0.0f;
  acc0[1] = 0.0f;
  acc0[2] = 0.0f;
  acc0[3] = 0.0f;
  ...
  for (var ridx3004 = 0; ridx3004 < 64; ridx3004++) {
    ...
    var val5 = data2_131072[alu12];
    var val6 = data2_131072[(alu12+2048)];
    var val7 = data2_131072[(alu12+4096)];
    var val8 = data2_131072[(alu12+6144)];
    acc0[0] = (acc0[0]+(val4*val5));
    acc0[1] = (acc0[1]+(val4*val6));
    acc0[2] = (acc0[2]+(val4*val7));
    acc0[3] = (acc0[3]+(val4*val8));
  ...
  if (((bool(lidx1))!=true)) {
    ...
    var alu33 = (acc1[2]+val2);
    var alu34 = (acc1[3]+val3);
    data0_256[alu4] = (alu31*(1/(1.0f+exp2((alu31*-1.4426950408889634f)))));
    data0_256[alu5] = (alu32*(1/(1.0f+exp2((alu32*-1.4426950408889634f)))));
    data0_256[alu6] = (alu33*(1/(1.0f+exp2((alu33*-1.4426950408889634f)))));
    data0_256[alu7] = (alu34*(1/(1.0f+exp2((alu34*-1.4426950408889634f)))));
  }
}`;

const r_8_4_4_8_32 = `fn nan() -> f32 { let bits = 0xffffffffu; return bitcast<f32>(bits); }
@group(0) @binding(0)
var<uniform> INFINITY : f32;
var<workgroup> temp0: array<f32,128>;
...
@compute @workgroup_size(4,8) fn main(@builtin(workgroup_id) gindex: vec3<u32>,@builtin(local_invocation_id) lindex: vec3<u32>) {
  ...
  acc0[2] = 0.0f;
  acc0[3] = 0.0f;
  var precast4 = (cast1<<12u);
  var precast5 = (cast0<<8u);
  for (var ridx3004 = 0; ridx3004 < 32; ridx3004++) {
    var precast6 = ridx3004;
    var precast7 = (bitcast<u32>(precast6)<<3u);
    var cast3 = bitcast<i32>(precast7);
    var val4 = data1_256[(lidx1+cast3)];
    var alu12 = (lidx1+bitcast<i32>(precast4)+bitcast<i32>(precast5)+cast3);
    var val5 = data2_32768[alu12];
    var val6 = data2_32768[(alu12+1024)];
    var val7 = data2_32768[(alu12+2048)];
    var val8 = data2_32768[(alu12+3072)];
    acc0[0] = (acc0[0]+(val4*val5));
    acc0[1] = (acc0[1]+(val4*val6));
    acc0[2] = (acc0[2]+(val4*val7));
    acc0[3] = (acc0[3]+(val4*val8));
...
const setupNet = async (device, safetensor) => {
    ...
    const buf_4 = createWeightBuf(device, 524288, getTensorBuffer(safetensor, metadata['layers.3.weight']));
    const buf_5 = createWeightBuf(device, 1024, getTensorBuffer(safetensor, metadata['layers.3.bias']));
    const buf_6 = createEmptyBuf(device, 512);;
    const buf_7 = createWeightBuf(device, 131072, getTensorBuffer(safetensor, metadata['layers.5.weight']));
    const buf_8 = createWeightBuf(device, 512, getTensorBuffer(safetensor, metadata['layers.5.bias']));
    const output0 = createEmptyBuf(device, 40);;
    const buf_9 = createWeightBuf(device, 5120, getTensorBuffer(safetensor, metadata['layers.7.weight']));
    const buf_10 = createWeightBuf(device, 40, getTensorBuffer(safetensor, metadata['layers.7.bias']));

    const gpuWriteBuffer0 = device.createBuffer({size:input0.size, usage: GPUBufferUsage.COPY_SRC | GPUBufferUsage.MAP_WRITE });

    const gpuReadBuffer0 = device.createBuffer({size:output0.size, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });

    const kernels = [r_32_4_4_8_98, r_16_4_4_8_64, r_8_4_4_8_32, r_10_16_8];
    ...
    return async (_input0) => {
        ...
        addComputePass(device, commandEncoder, pipelines[1], layouts[1], infinityBuf, [buf_3, buf_0, buf_4, buf_5], [16, 1, 1]);
        addComputePass(device, commandEncoder, pipelines[2], layouts[2], infinityBuf, [buf_6, buf_3, buf_7, buf_8], [8, 1, 1]);
        addComputePass(device, commandEncoder, pipelines[3], layouts[3], infinityBuf, [output0, buf_6, buf_9, buf_10], [10, 1, 1]);
